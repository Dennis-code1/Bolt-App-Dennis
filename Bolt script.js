  ...
  savedPlaces: [
    {
      ...
      label: "home",
      name: "Home",
      address: "123 Main Street, Osu, Accra",
    },
    {
      id: 2,
      label: "work",
      name: "Work",
      address: "456 Business Plaza, Legon, Accra",
    },
    {
      id: 3,
      label: "other",
      name: "Gym",
      address: "789 Fitness Center, Madina, Accra",
    },
  ],
...
async function estimateRideFareFromRoute(
  type,
  pickupAddress,
  destinationAddress,
) {
...
async function updateFareEstimate() {
  ...
  if (!pickupAddress || !destinationAddress) {
    ...
    fareDistanceText.textContent =
      "Pricing changes based on distance, pickup, and drop-off locations.";
    if (navigateBtn) navigateBtn.style.display = "none";
    return;
  }

  try {
    const quote = await estimateRideFareFromRoute(
      rideType,
      pickupAddress,
      destinationAddress,
    );
    if (!quote) {
      ...
      fareDistanceText.textContent =
        "Please enter a more specific pickup and destination.";
      if (navigateBtn) navigateBtn.style.display = "none";
      return;
    }
    ...
    const routeReady = await updateRideMapFromAddresses(
      pickupAddress,
      destinationAddress,
    );
    ...
    fareEstimateValue.textContent = "Could not estimate";
    fareDistanceText.textContent =
      "Please check your pickup and destination details.";
    if (navigateBtn) navigateBtn.style.display = "none";
  }
}
...
const mockDatabase = {
  users: [
    {
      id: 1,
      email: "aloysioudennis09@gmail.com",
      password: "123456",
      name: "Aloysious Dennis",
      phone: "0535915543",
      type: "rider",
      rating: 4.8,
      totalRides: 12,
  ...
  rides: [
    ...
    },
    {
      id: 2,
      from: "Tema, Accra",
      to: "Kasoa, Central Region",
      fare: 45.0,
      date: "2026-08-12",
      driver: "Samuel Agyeman",
      rating: 4,
    },
    {
      id: 3,
...
function populateQuickBookingFromInputs() {
  ...
  const dropoff =
    document.getElementById("quickDestination")?.value?.trim() || "";

  if (!pickup || !dropoff) {
    showNotification(
      "Please enter both a pickup and destination to continue.",
      "error",
    );
    return false;
  }
...
async function launchRideBookingFlow() {
  ...
  const dropoffLocation =
    document.getElementById("dropoffLocation")?.value?.trim() || "";
  const rideType = document.getElementById("rideType")?.value || "economy";
  const passengers = document.getElementById("passengers")?.value || "1";

  if (!pickupLocation || !dropoffLocation) {
    showNotification(
      "Please enter valid pickup and drop-off locations.",
      "error",
    );
    return false;
  }
  ...
  try {
    const routeQuote = await estimateRideFareFromRoute(
      rideType,
      pickupLocation,
      dropoffLocation,
    );
    ...
    showNotification(
      "Map lookup failed. Please enter a more specific address.",
      "error",
    );
    return false;
  }
...
function preparePaymentFromCurrentRide() {
  ...
  if (distanceFareEl)
    distanceFareEl.textContent = formatGhanaCurrency(distanceAmount);
  if (totalAmountEl)
    totalAmountEl.textContent = formatGhanaCurrency(totalAmount);
...
function getFallbackLocationCoords(query) {
  ...
  const cityCenters = {
    accra: {
      lat: 5.6037,
      lon: -0.187,
      displayName: "Accra, Greater Accra, Ghana",
    },
    osu: { lat: 5.5573, lon: -0.186, displayName: "Osu, Accra, Ghana" },
    "kotoka airport": {
      lat: 5.6044,
      lon: -0.1668,
      displayName: "Kotoka International Airport, Accra, Ghana",
    },
    airport: {
      lat: 5.6044,
      lon: -0.1668,
    ...
    tema: {
      lat: 5.6698,
      lon: -0.0164,
      displayName: "Tema, Greater Accra, Ghana",
    },
    kasoa: {
      lat: 5.5344,
      lon: -0.427,
      displayName: "Kasoa, Central Region, Ghana",
    },
    kumasi: {
      lat: 6.6885,
      lon: -1.6232,
      displayName: "Kumasi, Ashanti, Ghana",
    },
    ejisu: { lat: 6.7145, lon: -1.3786, displayName: "Ejisu, Ashanti, Ghana" },
    cape: {
      lat: 5.1053,
      lon: -1.2466,
      displayName: "Cape Coast, Central Region, Ghana",
    },
    "cape coast": {
      lat: 5.1053,
      lon: -1.2466,
    ...
    takoradi: {
      lat: 4.8935,
      lon: -1.76,
      displayName: "Takoradi, Western Region, Ghana",
    },
    sunyani: {
      lat: 7.3398,
      lon: -2.326,
      displayName: "Sunyani, Bono Region, Ghana",
    },
    elmina: {
      lat: 5.0839,
      lon: -1.35,
      displayName: "Elmina, Central Region, Ghana",
    },
    ...
    "accra mall": {
      lat: 5.637,
      lon: -0.1535,
      displayName: "Accra Mall, Accra, Ghana",
    },
    mall: { lat: 5.637, lon: -0.1535, displayName: "Accra Mall, Accra, Ghana" },
    kaneshie: {
      lat: 5.5714,
      lon: -0.2124,
      displayName: "Kaneshie, Accra, Ghana",
    },
    circle: { lat: 5.5607, lon: -0.206, displayName: "Circle, Accra, Ghana" },
  };
  ...
  if (firstWord && firstWord.length > 2) {
    return {
      ...
      lon: -0.187 + (firstWord.charCodeAt(firstWord.length - 1) % 10) * 0.05,
...
function offsetDriverPoint(baseCoords, index) {
  ...
  const latOffset =
    (distanceKm / earthRadiusKm) * (180 / Math.PI) * Math.cos(bearing);
  const lngOffset =
    ((distanceKm / earthRadiusKm) * (180 / Math.PI) * Math.sin(bearing)) /
    Math.cos(baseCoords.lat * (Math.PI / 180));
...
function drawNearbyDriversOnMap(baseCoords, drivers = mockDatabase.drivers) {
  ...
  mapState.nearbyDriverMarkers.forEach((marker) =>
    mapState.rideMap.removeLayer(marker),
  );
  mapState.nearbyDriverMarkers = [];

  drivers.forEach((driver, index) => {
    const markerCoords = offsetDriverPoint(baseCoords, index);
    const marker = L.marker([markerCoords.lat, markerCoords.lon]).addTo(
      mapState.rideMap,
    );
    marker.bindPopup(
      `<strong>${driver.name}</strong><br>${driver.car}<br>${driver.distance}`,
    );
...
async function updateRideMapFromAddresses(pickupAddress, destinationAddress) {
  ...
  if (!pickupResult || !destinationResult) {
    showNotification(
      "Could not find both locations on the map. Try more specific addresses.",
      "error",
    );
    return false;
  }
...
document
  ...
  ?.addEventListener("change", updateAccountTypeFields);
...
document.getElementById("loginForm").addEventListener("submit", (e) => {
  ...
  if (user.twoFactor) {
    ...
    if (!loginCode) {
      showNotification(
        `Two-factor authentication is enabled. Use the 6-digit code: ${requiredCode}`,
        "info",
      );
      return;
    }
    ...
    if (loginCode !== requiredCode) {
      showNotification(
        `Wrong security code. Demo code: ${requiredCode}`,
        "error",
      );
      return;
    }
...
document.getElementById("signupForm").addEventListener("submit", (e) => {
  ...
  if (!isValidGhanaPhone(phone)) {
    showValidationError(
      phoneInput,
      "Use a valid Ghana phone number like 0551234567.",
    );
    return;
  }
  ...
  if (accountType === "driver") {
    ...
    if (
      ...
      showNotification(
        "Please complete the driver details before registering.",
        "error",
      );
      return;
    }
...
function isValidGhanaPhone(phone) {
  return /^(\+233|233|0)(20|24|26|27|28|50|54|55|57|59)\d{7}$/.test(
    phone.replace(/\s+/g, ""),
  );
}
...
function showValidationError(input, message) {
  ...
  const existingMessage = input.parentElement?.parentElement?.querySelector(
    ".validation-message",
  );
  if (existingMessage) {
...
async function populateAvailableDrivers() {
  ...
  const baseCoords = appState.currentRide
    ? await geocodeLocation(appState.currentRide.pickup)
    : { lat: 5.6037, lon: -0.187 };

  mockDatabase.drivers.forEach((driver, index) => {
    const driverRate = Math.max(
      18,
      appState.currentRide
        ? getDynamicFare(
            appState.currentRide.type,
            routeDistance + driver.id * 0.8,
          )
        : 20,
    );
...
function selectDriver(driver, selectedFare) {
  ...
  document.getElementById("driverRatingActive").textContent =
    `⭐ ${driver.rating} (250 rides)`;
  document.getElementById("activePickup").textContent =
    appState.currentRide.pickup;
  document.getElementById("activeDropoff").textContent =
    appState.currentRide.dropoff;
...
function startRideSimulation() {
  ...
  const countdownInterval = setInterval(() => {
    if (arrivalTime <= 0) {
      ...
      showNotification(
        "Your driver has arrived. Please complete the ride payment.",
        "success",
      );
...
function simulateRideRequests() {
  ...
  if (requests.length === 0) {
    rideRequestsList.innerHTML =
      '<p class="no-requests">No ride requests at the moment</p>';
  }
}
...
function acceptRideRequest(user, from, to) {
  ...
  const currentEarnings =
    parseFloat(document.getElementById("driverEarnings").textContent) || 0;
  const newEarnings = currentEarnings + 20.5; // Mock earning
  document.getElementById("driverEarnings").textContent =
    `GHS ${newEarnings.toFixed(2)}`;

  // Update completed rides
  const completedRides =
    parseInt(document.getElementById("completedRides").textContent) + 1;
...
document
  ...
  .addEventListener("change", function () {
    ...
    if (momoFields)
      momoFields.style.display = shouldShowMomo ? "block" : "none";
    if (cardFields)
...
document.getElementById("paymentForm").addEventListener("submit", (e) => {
  ...
  showNotification(
    `Payment of ${totalAmount} processed successfully!`,
    "success",
  );
...
function loadProfileData() {
  ...
  if (securityLoginEmail)
    securityLoginEmail.textContent = user.email || "Not provided";
  ...
  if (totalRidesStats)
    totalRidesStats.textContent =
      user.type === "rider"
        ? String(user.totalRides)
        : String(user.completedRides || 0);
  if (ratingStats) ratingStats.textContent = `${user.rating} ⭐`;
  if (spentStats)
    spentStats.textContent = `GH₵${(user.type === "rider" ? user.totalSpent : user.totalEarnings || 0).toFixed(2)}`;
  ...
  lists.forEach((list) => {
    ...
    list.querySelectorAll('[data-action="delete"]').forEach((button) => {
      button.addEventListener("click", () => {
        ...
        appState.savedPlaces = appState.savedPlaces.filter(
          (place) => place.id !== id,
        );
        renderSavedPlaces();
...
if (addAddressForm) {
  addAddressForm.addEventListener("submit", (e) => {
    ...
    if (!label || !address) {
      showNotification(
        "Please select a label and enter a full address",
        "error",
      );
      return;
    }

    const editedId = addAddressForm.dataset.editId
      ? Number(addAddressForm.dataset.editId)
      : null;
...
if (viewCalendarBtn) {
  viewCalendarBtn.addEventListener("click", () => {
    activityCalendar.style.display =
      activityCalendar.style.display === "none" ? "block" : "none";
    renderCalendar();
  });
}
...
if (twoFactorToggle) {
  twoFactorToggle.addEventListener("change", () => {
    if (!appState.user) {
      ...
      showNotification(
        "Please login first to enable Face Lock security",
        "info",
      );
      return;
    }
...
if (biometricLoginBtn) {
  biometricLoginBtn.addEventListener("click", () => {
    ...
    if (!user.biometricLogin) {
      showNotification(
        "Biometric Login has not been enabled on this account yet.",
        "info",
      );
      return;
    }
...
if (personalEditForm) {
  personalEditForm.addEventListener("submit", (e) => {
    ...
    if (!name || !phone || !dob || !gender) {
      showNotification(
        "Please complete all personal information fields before saving.",
        "error",
      );
      return;
    }
    ...
    document.getElementById("displayDOB").textContent = new Date(
      dob,
...
if (profilePhotoInput) {
  profilePhotoInput.addEventListener("change", (event) => {
    ...
    if (!file || !appState.user) {
      showNotification(
        "Please log in before uploading a profile picture.",
        "info",
      );
      return;
    }
...
[rideTypeSelect, pickupLocationInput, dropoffLocationInput].forEach(
  (element) => {
    if (element) {
...
if (navigateRouteBtn) {
  navigateRouteBtn.addEventListener("click", () => {
    const origin =
      navigateRouteBtn.dataset.origin || pickupLocationInput?.value || "";
    const destination =
      navigateRouteBtn.dataset.destination || dropoffLocationInput?.value || "";

    if (!origin || !destination) {
      showNotification(
        "Please add both pickup and drop-off locations first.",
        "error",
      );
      return;
    }
...
if (deleteAccountBtn) {
  deleteAccountBtn.addEventListener("click", () => {
    if (
      ...
      const remainingUsers = mockDatabase.users.filter(
        (user) => user.email !== appState.user?.email,
      );
...
const cityInfo = {
  accra: {
    title: "Accra",
    subtitle:
      "The energetic heart of Ghana, full of life, music, and coastal charm.",
    ...
    highlights: [
      ...
      "Busy markets and local culture",
    ],
    image: "Acca.jpg",
  },
  kumasi: {
    title: "Kumasi",
    subtitle:
      "A cultural treasure known for tradition, craft, and warm hospitality.",
    ...
    highlights: [
      ...
      "Friendly people and lively streets",
    ],
  ...
  tema: {
    title: "Tema",
    subtitle:
      "A fast-growing port city that connects movement, trade, and opportunity.",
  ...
  "cape-coast": {
    title: "Cape Coast",
    subtitle:
      "A historic coastal city with beauty, heritage, and peaceful ocean views.",
  ...
  takoradi: {
    title: "Takoradi",
    subtitle:
      "A lively sea city filled with energy, trade, and breathtaking coastline.",
  ...
  kakum: {
    title: "Kakum",
    subtitle:
      "A gateway to nature, rainforest adventure, and fresh green beauty.",
    ...
    highlights: [
      ...
      "Nature and canopy walks",
      "Eco-tourism and scenic views",
    ],
    image: "unnamed.webp",
  },
  sunyani: {
    title: "Sunyani",
    subtitle:
      "A delightful city of friendliness, calm surroundings, and inner-city warmth.",
  ...
  elmina: {
    title: "Elmina",
    subtitle:
      "A historic coastal town where heritage and beauty live together.",
    ...
    highlights: [
      "Historic castle heritage",
      "Scenic sea views",
      "Quiet coastal beauty",
    ],
    image: "68.jpg",
  },
};
...
function openCityModal(cityKey) {
  ...
  highlights.innerHTML = city.highlights
    .map((item) => `<li>${item}</li>`)
    .join("");
...
document
  .querySelectorAll('[data-close-city-modal="true"]')
  .forEach((element) => {
...
if (themeModeToggle) {
  themeModeToggle.addEventListener("change", () => {
    ...
    showNotification(
      themeModeToggle.checked ? "Night mode enabled" : "Day mode enabled",
      "info",
    );
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Map buttons to section IDs
  const navMappings = [
    { id: "safetyBtn", target: "safetySection" },
    { id: "helpBtn", target: "helpCenterSection" },
    { id: "contactBtn", target: "footerSupport" },
    { id: "rideBtn", target: "ridePage" },
    { id: "driveBtn", target: "driverPage" },
    // Add more mappings as needed
  ];

  navMappings.forEach(({ id, target }) => {
...
document.getElementById("safetyBtn").addEventListener("click", () => {
  window.location.href = "safety.html"; // replace with your actual page URL
});
// Map buttons to section IDs
const navMappings = [
  { id: "safetyBtn", target: "safetySection" },
...
// Show modal
document.getElementById("moreSafetyInfoBtn").addEventListener("click", () => {
